...
public class GnomeForestShortTreesBiome extends BpgCustomizationsModElements.ModElement {
	public static Biome biome;
	public GnomeForestShortTreesBiome(BpgCustomizationsModElements instance) {
		super(instance, 46);
	...
	private static class BiomeRegisterHandler {
		...
		public void registerBiomes(RegistryEvent.Register<Biome> event) {
			if (biome == null) {
				BiomeAmbience effects = new BiomeAmbience.Builder().setFogColor(-4949838).setWaterColor(-14540853).setWaterFogColor(-14540853)
						.withSkyColor(-4949838).withFoliageColor(-2380288).withGrassColor(-13466368)
				...
				biome = new Biome.Builder().precipitation(Biome.RainType.RAIN).category(Biome.Category.FOREST).depth(0.2f).scale(0.3f)
						.temperature(0.5f).downfall(0.5f).setEffects(effects).withMobSpawnSettings(mobSpawnInfo.copy())
						.withGenerationSettings(biomeGenerationSettings.build()).build();
				event.getRegistry().register(biome.setRegistryName("bpg_customizations:gnome_forest_short_trees"));
			}
		}
	}
	@Override
	...
	private static class CustomLeaveVineTreeDecorator extends LeaveVineTreeDecorator {
		...
		static {
			...
			tdt.setRegistryName("gnome_forest_short_trees_lvtd");
	...
	private static class CustomTrunkVineTreeDecorator extends TrunkVineTreeDecorator {
		...
		static {
			codec = com.mojang.serialization.Codec.unit(() -> instance);
			tdt = new TreeDecoratorType(codec);
			tdt.setRegistryName("gnome_forest_short_trees_tvtd");
			ForgeRegistries.TREE_DECORATOR_TYPES.register(tdt);
		}
		@Override
		protected TreeDecoratorType<?> func_230380_a_() {
