...
public class GnomeMountainsBiome extends BpgCustomizationsModElements.ModElement {
	public static Biome biome;
	public GnomeMountainsBiome(BpgCustomizationsModElements instance) {
		super(instance, 49);
			...
			if (biome == null) {
				...
				biomeGenerationSettings.withFeature(GenerationStage.Decoration.VEGETAL_DECORATION, Feature.TREE
						.withConfiguration((new BaseTreeFeatureConfig.Builder(new SimpleBlockStateProvider(Blocks.DARK_OAK_LOG.getDefaultState()),
								...
								new StraightTrunkPlacer(1, 2, 0), new TwoLayerFeature(1, 0, 1))).setIgnoreVines().setMaxWaterDepth(0).build())
				...
				biome = new Biome.Builder().precipitation(Biome.RainType.SNOW).category(Biome.Category.NONE).depth(2f).scale(0.3f).temperature(0.1f)
						.downfall(0.3f).setEffects(effects).withMobSpawnSettings(mobSpawnInfo.copy())
						.withGenerationSettings(biomeGenerationSettings.build()).build();
				event.getRegistry().register(biome.setRegistryName("bpg_customizations:gnome_mountains"));
			}
		}
	}
	@Override
