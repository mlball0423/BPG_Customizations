...
package net.mcreator.bpgcustomizations.world.dimension;
...
import net.mcreator.bpgcustomizations.item.GnomiaItem;
import net.mcreator.bpgcustomizations.block.HyperiumBlockBlock;
...
public class GnomiaDimension extends BpgCustomizationsModElements.ModElement {
	@ObjectHolder("bpg_customizations:gnomia_portal")
	public static final CustomPortalBlock portal = null;
	public GnomiaDimension(BpgCustomizationsModElements instance) {
		super(instance, 23);
	...
	public void clientLoad(FMLClientSetupEvent event) {
		...
		DeferredWorkQueue.runLater(() -> {
			try {
				...
				effectsRegistry.put(new ResourceLocation("bpg_customizations:gnomia"), customEffect);
			} catch (Exception e) {
				e.printStackTrace();
			}
		});
	...
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("gnomia_portal", Vector3i::compareTo, 300);
	public static class POIRegisterHandler {
		...
		public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
			poi = new PointOfInterestType("gnomia_portal", Sets.newHashSet(ImmutableSet.copyOf(portal.getStateContainer().getValidStates())), 0, 1)
					.setRegistryName("gnomia_portal");
	...
	public void initElements() {
		...
		elements.items.add(() -> new GnomiaItem().setRegistryName("gnomia"));
	}
	public static class CustomPortalBlock extends NetherPortalBlock {
		...
		public void animateTick(BlockState state, World world, BlockPos pos, Random random) {
			for (int i = 0; i < 4; i++) {
				...
				world.addParticle(ParticleTypes.LANDING_OBSIDIAN_TEAR, px, py, pz, vx, vy, vz);
			}
			...
			if (!entity.isPassenger() && !entity.isBeingRidden() && entity.isNonBoss() && !entity.world.isRemote && true) {
				if (entity.func_242280_ah()) {
					entity.func_242279_ag();
						...
						new ResourceLocation("bpg_customizations:gnomia"))) {
					entity.func_242279_ag();
					teleportToDimension(entity, pos,
							RegistryKey.getOrCreateKey(Registry.WORLD_KEY, new ResourceLocation("bpg_customizations:gnomia")));
				} else {
					entity.func_242279_ag();
	...
	public static class CustomPortalSize {
		private static final AbstractBlock.IPositionPredicate POSITION_PREDICATE = (state, blockReader, pos) -> {
			return state.getBlock() == HyperiumBlockBlock.block.getDefaultState().getBlock();
		};
					...
					for (int k2 = 0; k2 < 2; ++k2) {
						for (int i3 = -1; i3 < 3; ++i3) {
							BlockState blockstate1 = i3 < 0
									? HyperiumBlockBlock.block.getDefaultState().getBlock().getDefaultState()
									: Blocks.AIR.getDefaultState();
			...
			for (int k1 = -1; k1 < 3; ++k1) {
				for (int i2 = -1; i2 < 4; ++i2) {
					if (k1 == -1 || k1 == 2 || i2 == -1 || i2 == 3) {
						...
						this.world.setBlockState(blockpos$mutable, HyperiumBlockBlock.block.getDefaultState().getBlock().getDefaultState(), 3);
					}
				}
			}
